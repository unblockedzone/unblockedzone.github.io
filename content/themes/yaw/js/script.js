...
$(function () {
  ...
  async function fetchMoreGames(amount, sort_by) {
    try {
      // Lấy dữ liệu từ sorted_data.json
      const response = await fetch("sorted_data.json");
      const data = await response.json();

      // Sắp xếp dữ liệu nếu cần (nếu sort_by được sử dụng)
      if (sort_by) {
        data.sort((a, b) => {
          if (sort_by === "id") {
            return a.id - b.id; // Sắp xếp theo id
          }
          // Có thể thêm các điều kiện sắp xếp khác ở đây
          return 0;
        });
      }

      // Chọn một phần dữ liệu dựa trên amount
      const fetchedGames = data.slice(last_offset, last_offset + amount);

      // Cập nhật last_offset cho lần gọi tiếp theo
      last_offset += fetchedGames.length;

      // Thêm các game đã lấy vào giao diện
      appendFetchedGames(fetchedGames);
    } catch (error) {
      console.log(error);
    }
  }
